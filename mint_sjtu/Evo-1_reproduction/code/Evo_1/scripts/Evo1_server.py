...
def load_model_and_normalizer(ckpt_dir):
    ...
    vlm_dir = os.environ.get("EVO1_VLM_DIR")
    if vlm_dir:
        vlm_dir = os.path.abspath(os.path.expanduser(vlm_dir))
        if not os.path.isdir(vlm_dir):
            raise ValueError(f"EVO1_VLM_DIR does not exist or is not a directory: {vlm_dir}")
        config["vlm_name"] = vlm_dir
...
if __name__ == "__main__":
    ...
    if ckpt_dir == "Your/Path/To/Checkpoint" or not os.path.isdir(ckpt_dir):
        raise ValueError(
            "Set EVO1_CKPT_DIR to a valid checkpoint directory "
            "(for example: /home/Travor/work/Evo-1/checkpoints/libero)."
        )

    port = int(os.environ.get("EVO1_PORT", "9000"))
